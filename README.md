...
onRecoverPassword |   `RecoverCallback`     | <sub>Called when the user hit the submit button when in recover senha mode</sub>
...
loginValidator |   <sub>`FormFieldValidator<String>`</sub>     | <sub>Login validating logic, Returns an error string to display if the input is invalid, or null otherwise</sub>
senhaValidator | <sub>`FormFieldValidator<String>`</sub>     | <sub>Same as `loginValidator` but for senha</sub>
...
senhaHint | `String` | Hint text of the senha [TextField]
confirmPasswordHint | `String` | Hint text of the confirm senha [TextField]
forgotPasswordButton | `String` | Forgot senha button's label
loginButton | `String` | Login button's label
signupButton | `String` | Signup button's label
recoverPasswordButton | `String` | Recover senha button's label
recoverPasswordIntro | `String` | Intro in senha recovery form
recoverPasswordDescription | `String` | Description in senha recovery form
goBackButton | `String` | Go back button's label. Go back button is used to go back to to login/signup form from the recover senha form
confirmPasswordError | `String` | The error message to show when the confirm senha not match with the original senha
recoverPasswordSuccess | `String` | The success message to show after submitting recover senha
...
bodyStyle | `TextStyle` | Text style for small text like the recover senha description
...
class LoginScreen extends StatelessWidget {
  ...
  Future<String> _authUser(LoginUpload data) {
    print('Name: ${data.name}, Password: ${data.senha}');
    return Future.delayed(loginTime).then((_) {
      if (!users.containsKey(data.name)) {
        return 'Username not exists';
      }
      if (users[data.name] != data.senha) {
      ...
      theme: LoginTheme(
        ...
        inputTheme: InputDecorationTheme(
          ...
          disabledBorder: UnderlineInputBorder(
            borderSide: BorderSide(color: theme.primaryColor, width: 5),
            borderRadius: inputBorder,
          ),
        ),
